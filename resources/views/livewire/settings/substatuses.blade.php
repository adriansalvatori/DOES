<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="bg-white border border-[#e9e9e7] rounded-2xl p-5 shadow-2xs flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold text-zinc-900 tracking-tight">Configuración de Subestatus</h1>
                <p class="text-xs text-zinc-500 mt-0.5">Crea, personaliza y asigna códigos de color para los subestatus de las órdenes.</p>
            </div>
        </div>
    ...
    <div class="bg-white border border-[#e9e9e7] rounded-2xl shadow-2xs overflow-hidden">
        <div class="px-5 py-4 border-b border-[#e9e9e7] flex items-center justify-between bg-stone-50/50">
            <h2 class="font-bold text-xs text-zinc-800 uppercase tracking-wider">Lista de Subestatus ({{ $substatuses->count() }})</h2>
            <span class="text-[11px] text-zinc-400">Los cambios se aplican automáticamente en todas las vistas</span>
        </div>

        @if($substatuses->isEmpty())
            <div class="p-12 text-center text-zinc-400">
                <x-lucide-tag class="w-10 h-10 mx-auto text-zinc-300 mb-2" />
                <p class="text-xs">No se encontraron subestatus que coincidan con la búsqueda.</p>
            </div>
        @else
                    ...
                    <div class="px-5 py-3.5 flex items-center justify-between hover:bg-stone-50/60 transition gap-4">
                        ...
                        <div class="hidden sm:flex items-center gap-2 shrink-0">
                            <span class="text-[10px] px-2 py-0.5 rounded border font-medium {{ $sub->style_type === 'solid' ? 'bg-stone-800 text-white border-stone-900' : 'bg-stone-100 text-zinc-600 border-stone-200' }}">
                                {{ $sub->style_type === 'solid' ? 'Color Sólido' : 'Fondo Claro' }}
                            </span>
                        </div>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            @if(!$sub->is_system)
                                <button 
                                    ...
                                    wire:confirm="¿Estás seguro de eliminar el subestatus '{{ $sub->name }}'?" 
            ...
            <div class="bg-white rounded-2xl border border-stone-200 shadow-xl max-w-md w-full p-6 space-y-5 animate-in fade-in zoom-in duration-150">
                <div class="flex items-center justify-between border-b border-stone-100 pb-3">
                    <h3 class="font-bold text-sm text-zinc-900 flex items-center gap-2">
                        ...
                        <span>{{ $editingId ? 'Editar Subestatus' : 'Nuevo Subestatus' }}</span>
                    </h3>
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    <div>
                        <label class="block text-xs font-medium text-zinc-700 mb-1">Nombre del Subestatus</label>
                        <input 
                            wire:model="name" 
                            type="text" 
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-medium text-zinc-700">Estilo de Fondo</label>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-medium text-zinc-700">Color Principal del Subestatus</label>
                        ...
                        <div class="pt-2">
                            <span class="text-[11px] font-medium text-zinc-600 block mb-1.5">Paleta del Proyecto:</span>
                    ...
                    <div class="p-4 bg-stone-50 border border-stone-200 rounded-xl space-y-1.5">
                        <span class="text-[10px] text-zinc-400 uppercase font-semibold block">Vista Previa</span>
                        <div class="pt-0.5">
                            <span 
                                ...
                                {{ $name ? strtoupper($name) : 'EJEMPLO SUBESTATUS' }}
                            </span>
                        </div>
                    </div>
